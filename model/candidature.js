...
module.exports = {
    ...
    db.query("select * from Candidature where num_offre=? and email_candidat=?",[num_offre,email_candidat],function
    (err, results) {
    if (err) throw err;
        callback(results);
    });
    ...
    sql = "SELECT email_candidat FROM Candidature WHERE num_offre =? and email_candidat=?";
    ...
    //todo
        sql = "Insert into Candidature(num_offre,email_candidat,date, lettre_motivation,CV,ID,photo,diplôme) Values (?,?,?,?,?,?,?,?)";
        ...
        update: function(num_offre,email_candidat,nom, type,siège_social,statut, callback) {
            const sql = 'UPDATE Candidature SET num_offre =?  and email_candidat=? WHERE id_fiche = ?';
